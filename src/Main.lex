...
"skip" { tokens.add("skip"); return new Symbol(sym.SKIP); }

"repeat" { tokens.add("repeat"); return new Symbol(sym.REPEAT); }
"until" { tokens.add("until"); return new Symbol(sym.UNTIL); }
...
{INTEGER} { tokens.add(String.format("num(%d)", new Integer(yytext()))); return new Symbol(sym.INTEGER, new Integer(yytext())); }
{BOOLEAN} { tokens.add(yytext()); return new Symbol(sym.BOOLEAN, new Boolean(yytext())); } // Tengo que poner BOOLEAN antes que IDENTIFIER porque si no, todo se lo asigna a IDENTIFIER
{IDENTIFIER} { tokens.add(String.format("ide(%s)", yytext())); return new Symbol (sym.IDENTIFIER, yytext().toUpperCase());  }
